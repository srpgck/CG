...
function getrndmcdla() {
  ...
  return "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSB1q7QyXG_r_lrGzkvfi80lA5TtgOOnX2_yA&usqp=CAU"; /*fetch(url)
  ...
  function Game() {
    this.newGame = function () {
      ...
      if (player.checkWager()) {
        ...
        showAlert("Olmayan parayı ne koyuyon?");
      }
    };
  }
  ...
  Player.prototype.insure = function () {
    ...
    if (this.checkWager()) {
      ...
      $("#alert")
        .removeClass("alert-info alert-success")
        .addClass("alert-error");
      showAlert("Sigorta için yeterli paran yok fakir.");
    }
  };
    ...
    if (pscore > dscore) {
      ...
      if (pscore <= 21 && dscore > 21) {
        ...
        $("#alert")
          .removeClass("alert-info alert-error")
          .addClass("alert-success");
        result = "Kazandın - Kasa fazla çekti (31)!";
      } else if (dscore <= 21) {
        winnings -= wager;
        player.setBank(winnings);
        $("#alert")
      ...
      if (pscore <= 21) {
        if (dscore === 21 && dhand.length < 3 && phand.length > 2) {
          ...
          $("#alert")
            .removeClass("alert-info alert-success")
            .addClass("alert-error");
          result = "Kaybettin - Kasa blackjack yaptı!";
        } else {
          winnings = wager;
          $("#alert")
    ...
    if (cash > 0 && !running) {
      if ($.trim($("#wager").val()) > 0) {
        ...
        $("#alert")
          .removeClass("alert-info alert-success")
          .addClass("alert-error");
        showAlert("Fakir. En düşük bet 1 dolardır.");
      }
    } else {
      $("#myModal").modal();
    }
